...
function updateHouseData(action) {
    ...
    $.post('/order/search_info/', params, function(data){
    ...
    updateFilterDateDisplay();
      $.get('/house/area_info/', function(data){
        for (var i=0; i< data[0].length; i++){
            if (data[0][i]["id"] == areaId){
                $('.filter-area').append("<li area-id='"+ data[0][i]["id"] +"' class='active' >"+ data[0][i]["name"] +"</li>");
            }
            else{
                $('.filter-area').append("<li area-id='"+ data[0][i]["id"] +"'>"+ data[0][i]["name"] +"</li>");
            }
        }
